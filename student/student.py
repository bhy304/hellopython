...
class Student:
    ...
    # print(list(map(lambda stu: stu.grading_students(), students)))
    ...
    def print_above_avg(self):
        print("4.평균 이상")
        std_avg = filter(lambda x : x.score >= self.avg, self.stu)
        for s in std_avg:
            print(s)
        # for s in students:
        #     if s.score >= avg:
...
Student.sorting_students()
Student.get_sum()
Student.print_above_avg()
